...
export async function sendResultEmail({
  ...
  const { data, error } = await resend().emails.send({
    ...
    subject,
    html: `
      <div style="font-family: Georgia, serif; max-width: 600px; margin: 0 auto; padding: 40px 20px; background: #F7F9FC; color: #12263F;">
        <h1 style="color: #0F7B7B; font-size: 24px; margin-bottom: 20px;">${escapeHtml(siteName)}</h1>
        ...
        <p style="font-size: 12px; color: #8A97A5;">
